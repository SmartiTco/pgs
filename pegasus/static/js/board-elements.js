...
var ImageElement = function(softID, canvasID, boardID, invite, canEdit){ // canEdit is boolean
	this.content = '';
	this.softID = softID;
	this.canvasID = canvasID;
	this.boardID = boardID;
	...
	this.init = function(message, id, pos){
		...
		self.position = pos;
		var jsonParse = JSON.parse(self.position);
		$(self.canvasID).prepend('<span class="draggable-image" id="image-'+self.elementID+'"></span>');
		var list = message.split('\n');
		var final = list.join('<br>');
		self.content = final;
		$('#image-'+self.elementID).html(final);
		$('#image-'+self.elementID).css(jsonParse);
		if(self.editable){
			$('#image-' + self.elementID).draggable({
				containment: self.canvasID,
				cursor: 'move',
				stop: function(event, ui){
					self.refresh(ui.position);
			...
			function hoverEnterEvent(){
				$('#image-delete-form-'+self.elementID).remove();
				$(this).prepend('<form class="delete-form" id="image-delete-form-'+self.elementID
							+'"><button type="submit" class="global-btn delete-btn" title="Delete Image" id="delete-'+self.elementID
							+'"><i class="fa fa-trash"></i></button></form>');
				$('#image-delete-form-'+self.elementID).bind('submit', function(event){
					event.preventDefault();
					var id = self.elementID;
					var csrf = $("input[name='_csrf_token']").val();
					var invite = self.invite;
			...
			function doubleClickEvent(){
				var v = $(this).image(); // this won't render line breaks so fix it
				$(this).html('<form id="image-edit-form-'+self.elementID
							+'"><textarea id="image-edit-'+self.elementID
							+'" name="message" class="image-edit-input">'+v+'</textarea></form>');
				$('#image-edit-'+self.elementID).focus();
				$('#image-edit-'+self.elementID).bind('blur', function(){
					$('#image-edit-form-'+self.elementID).submit();
				});
				$('#image-edit-'+self.elementID).keydown(function(event){
					if(event.keyCode==13 && !event.shiftKey){
						$('#image-edit-form-'+self.elementID).submit();
						return false;
					}
				});
				$('#image-edit-form-'+self.elementID).bind('submit', function(event){
					event.preventDefault();
					var val = $('#image-edit-'+self.elementID).val();
					self.save(val, 'edit', self.elementID);
				});

			}
			$('#image-' + self.elementID).dblclick(doubleClickEvent);
			// double tap event would go here

			$('#image-' + self.elementID).hover(hoverEnterEvent, hoverLeaveEvent);
			// single tap event would go here
		}
	...
	this.create = function(){
		var self = this;
		if(self.editable){
			$(self.canvasID).prepend('<form id="image-create-form-'+self.softID+'">'
								+'<imput type="file" name="image" id="image-create-'+self.softID+'" class="image-create-input"></imput>'
								+'</form>');
			$('#image-create-'+self.softID).focus();
			$('#image-create-'+self.softID).bind('blur', function(){
				$('#image-create-form-'+self.softID).submit();
			});
			$('#image-create-'+self.softID).keydown(function(event){
				if(event.keyCode==13 && !event.shiftKey){
						$('#image-create-form-'+self.softID).submit();
						return false;
					}
			});
			$('#image-create-form-'+self.softID).bind('submit', function(event){
				event.preventDefault();
				var val = $('input[name="image"]', this).val();
				self.save(val, 'create', self.softID);
			});
		}
		
	}
	this.save = function(image, type, id, pos){
		var self = this;
		if(self.editable){
			var URL;
			if(type=='create'){
			...
			$.ajax({
				...
				success: function(data){
					if(data.error == 'None'){
						var formSelector = '#image-'+type+'-form-'+id;
						$(formSelector).remove();
						if(type=='create'){
							self.init(message, data.componentID, position);
						}
